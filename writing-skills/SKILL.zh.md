...
- [ ] 内联代码或链接到单独文件
- [ ] 一个优秀的示例（非多语言）
- [ ] **语言规则：** `SKILL.md` 必须是英文。如果你需要其他语言（例如中文），请创建一个单独的 `<filename>.zh.md` 文件。
- [ ] **同步规则：** 确保 `SKILL.md` 的任何更新都立即反映在 `SKILL.zh.md` 中（反之亦然），以保持同步。
- [ ] **需求分析：** 在设计技能之前，识别 3 个关于技能需求和用户的“跨度大”且“有深度”的问题。在回答这些问题之前不要继续。
- [ ] 使用技能运行场景 - 验证代理现在是否遵守
