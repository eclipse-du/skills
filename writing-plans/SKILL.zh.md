...
**语言规则：** 无论用户使用何种语言输入，你 **必须** 用 **中文** 提问。
...
- 每次至多问 **3** 个问题。
- 如果用户的回答揭示了新的模糊点，请在下一轮中提出后续问题。
- **重复此循环**，直到你完全确信理解了需求。
- 只有到那时才开始编写计划。

**停止 (STOP):** 你必须使用 `notify_user` 来询问这些问题。在用户回复之前，你 **绝对不能** 生成计划。等待用户的回复。
...
**语言规则：** 如果用户使用中文沟通，这份文档必须 **全部用中文** 编写。
